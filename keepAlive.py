...
def func(function):
    x = Math.linspace(0,2*Math.pi, num=150)
    try:
        ...
        for word in result:
            ...
            num = "0" if regexp.sub("^((?!((\d)|(x)|(math.)+(\w{1,})\(x\)|(Math.)+(\w{1,})\(x\)|(Math.)+(\w{1,})|(math.)+(\w{1,}))).)*$","",word) == "" else regexp.sub("^((?!((\d)|(x)|(math.)+(\w{1,})\(x\)|(Math.)+(\w{1,})\(x\)|(Math.)+(\w{1,})|(math.)+(\w{1,}))).)*$","",word)
